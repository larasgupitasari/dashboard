...
<script>
    ...
    title: {
        text: 'Realisasi Anggaran Belanja Bulan Februari (%)'
    },
    tooltip: {
...
<script>
    Highcharts.chart('barChart', {
        ...
        title: {
            text: 'Penyerapan Anggaran Bulan Februari 2022'
        },
        xAxis: {
            type: 'category',
            labels: {
        ...
        series: [{
            ...
            data: [
                            <?php foreach ($Februari as $feb){
                                $nama = (string) $feb['nm_skpd'];
                                $prs = (float) number_format(($feb['realisasi']/$feb['anggaran']*100), 2, '.', '');
                                $data = [$nama,$prs];
...
<div class="container" style="margin-top: 55px">
    <div class="card mt-5">
        <div class="card-body">
            <h4>Realisasi Belanja Seluruh SKPD Berdasarkan Anggaran Kas Bulan Februari</h4>
            <br>
            <table id="tabel" class="table table-striped table-bordered" cellspacing="0" width="100%">
                ...
                <tbody>
                    <?php
                        $no=1;
                        foreach ($Februari as $feb): ?>
                        <tr>
                          <td><?php echo $no++;?>.</td>
                          <td><?= $feb['nm_skpd']; ?></td>
                          <td>Rp.<?= number_format(($feb['anggaran']), 0, ',','.'); ?></td>
                          <td>Rp.<?= number_format(($feb['realisasi']), 0, ',','.'); ?></td>
                          <td><?php echo number_format(($feb['realisasi']/$feb['anggaran']*100), 2, ',', ''); ?>%</td>
                          </tr>
                        <?php endforeach; ?>
                </tbody>
            </table>
