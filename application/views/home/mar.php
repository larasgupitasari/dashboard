...
<script>
    ...
    title: {
        text: 'Realisasi Anggaran Belanja Bulan Maret (%)'
    },
    tooltip: {
...
<script>
	Highcharts.chart('barChart', {
	    ...
	    title: {
	        text: 'Penyerapan Anggaran Bulan Maret 2022'
	    },
	    xAxis: {
	        type: 'category',
	        labels: {
	    ...
	    series: [{
	        ...
	        data: [
							<?php foreach ($Maret as $mar){
						     	$nama = (string) $mar['nm_skpd'];
							   	$prs = (float) number_format(($mar['realisasi']/$mar['anggaran']*100), 2, '.', '');
							   	$data = [$nama,$prs];
...
<div class="container" style="margin-top: 55px">
	<div class="card mt-5">
		<div class="card-body">
			<h4>Realisasi Belanja Seluruh SKPD Berdasarkan Anggaran Kas Bulan Maret</h4>
			<br>
			<table id="tabel" class="table table-striped table-bordered" cellspacing="0" width="100%">
	        	<thead style="text-align: center;">
	          	...
	          	<tbody>
					<?php
		            	$no=1;
		            	foreach ($Maret as $mar): ?>
		          		<tr>
			              <td><?php echo $no++;?>.</td>
			              <td><?= $mar['nm_skpd']; ?></td>
			              <td>Rp.<?= number_format(($mar['anggaran']), 0, ',','.'); ?></td>
			              <td>Rp.<?= number_format(($mar['realisasi']), 0, ',','.'); ?></td>
			              <td><?php echo number_format(($mar['realisasi']/$mar['anggaran']*100), 2, ',', ''); ?>%</td>
			         	  </tr>
			         	<?php endforeach; ?>
	      		</tbody>
	  		</table>
